...
#include <Base/System/BEK_Initializer.h>
...
void BERF_Init(BERT_Stacks* berv_sts) {
   ...
   berv_sts->bool_True = BEKF_5_4_LogicBool_new_0( 0, berv_sts, BERF_Create_Instance(berv_sts, berv_sts->passedClassDef, 0));
   
   berv_sts->passedClassDef = BEUV_5_4_LogicBool_clDef;
   berv_sts->bool_False = BEKF_5_4_LogicBool_new_0( 0, berv_sts, BERF_Create_Instance(berv_sts, berv_sts->passedClassDef, 0));
   
   berv_sts->passedClassDef = BEUV_5_5_LogicBools_clDef;
   berv_sts->bools_singleton = BEKF_6_11_SystemInitializer_initializeIt_1(0, berv_sts, NULL, BERF_Create_Instance(berv_sts, berv_sts->passedClassDef, 0));
   
   berv_sts->passedClassDef = BEUV_6_3_EncodeUrl_clDef;
   berv_sts->url_singleton = BEKF_6_11_SystemInitializer_initializeIt_1(0, berv_sts, NULL, BERF_Create_Instance(berv_sts, berv_sts->passedClassDef, 0));
   
   berv_sts->passedClassDef = BEUV_6_16_SystemExceptionBuilder_clDef;
   berv_sts->exceptionBuilder_singleton = BEKF_6_11_SystemInitializer_initializeIt_1(0, berv_sts, NULL, BERF_Create_Instance(berv_sts, berv_sts->passedClassDef, 0));
}
...
BERT_Slab* BERF_AddSlab(BERT_Stacks* berv_sts, BERT_Slab** allocSlabs, size_t allocBucket) {
   size_t slabSize;
   BERT_Slab* twrv_slab;
   size_t allocSize;
   ...
   twrv_slab = allocSlabs[allocBucket];
   
   while (twrv_slab != NULL && twrv_slab->next != NULL) {
      twrv_slab = twrv_slab->next;
   }
   allocSlabs[allocBucket] = twrv_slab;
   ...
   if (allocSlabs[allocBucket] != NULL) {
      allocSlabs[allocBucket]->next = twrv_slab;
      twrv_slab->prior = allocSlabs[allocBucket];
   }
   allocSlabs[allocBucket] = twrv_slab;
...
void** BERF_Create_Instance(BERT_Stacks* berv_sts, BERT_ClassDef* bevl_scldef, BEINT attempt) {
   ...
   size_t isize; /*the alloc size of the inst, for incrementing slab on create*/
   char* slab;
   void** cursor;
   BEUINT vmark;
   BEUINT maxVmark;
   ...
   if (bevl_scldef->allocBucket < berbmax) {
      /*the only place where allocation from a slab happens*/
      if (bevl_scldef->freeFirstSlot == 1) {
         allocSlabs = berv_sts->freeFirstAllocSlabs;
      } else {
         allocSlabs = berv_sts->allocSlabs;
      }
      twrv_slab = allocSlabs[bevl_scldef->allocBucket];
...
#endif
               /* Using an existing value from a slab - it may have been an instance before or may be from never before used space */
               /*need to free first slot before reuse if is free first and was previously used*/
               if (bevl_scldef->freeFirstSlot == 1 && cursor[bercps] != NULL) {
                BENoFree(cursor[bercps]);
                cursor[bercps] = NULL;
              }
               twvg = cursor;
               if (slab == NULL) {
                  if (twrv_slab->next == NULL) {
                     ...
                     allocSlabs[bevl_scldef->allocBucket] = twrv_slab;
                  } else {
                     twrv_slab->slabAt = twrv_slab->slab;
                     allocSlabs[bevl_scldef->allocBucket] = twrv_slab->next;
                  }
               } else {
      ...
      if (twvg == NULL) {
         ...
         twrv_slab = BERF_AddSlab(berv_sts, allocSlabs, bevl_scldef->allocBucket);
...
void** BERF_String_For_Chars(BERT_Stacks* berv_sts, char* cstr) {
   ...
   return BERF_String_For_Chars_Size(berv_sts, cstr, nchars);
}
...
void** BERF_String_For_Chars_Size(BERT_Stacks* berv_sts, char* cstr, BEINT nchars) {
   ...
   *((BEINT*) (((void**)(bevs[bercps + 1])) + bercps)) = nchars;
   bevs[bercps + 2] = BERF_Create_Instance(berv_sts, BEUV_4_3_MathInt_clDef, 0);
   berv_sts->passBack = bevs;
   *((BEINT*) (((void**)(bevs[bercps + 2])) + bercps)) = nchars;
   
   buf = (char*) BENoMalloc(nchars + 1);
   bevs[bercps] = buf;
...
char* BERF_Copy_Chars(char* cstr) {
   ...
   cstrcp = (char*) BENoMalloc(nchars + 1);
...
void BERF_Throw_CallOnNull(BERT_Stacks* berv_sts, char* clname, char* subname, char* fname, int line) {
   ...
   bevl_ex = BEKF_6_10_SystemCallOnNull_new_1(0, berv_sts, BERF_Create_Instance(berv_sts, berv_sts->passedClassDef, 0), bevl_msg);
...
void BERF_Throw_IncorrectType(BERT_Stacks* berv_sts, char* clname, char* subname, char* fname, int line) {
   ...
   bevl_ex = BEKF_6_13_SystemIncorrectType_new_1(0, berv_sts, BERF_Create_Instance(berv_sts, berv_sts->passedClassDef, 0), bevl_msg);
   berv_sts->passBack = bevl_ex;
   BERF_Throw(berv_sts, bevl_ex, clname, subname, fname, line);
}
...
void BERF_Throw(BERT_Stacks* berv_sts, void** _passBack, char* clname, char* subname, char* fname, int line) {
   ...
   bevl_line = BEKF_4_3_MathInt_new_0(0, berv_sts, BERF_Create_Instance(berv_sts, berv_sts->passedClassDef, 0));
...
void BERF_Collect(BERT_Stacks* berv_sts) {
   ...
   size_t decB = 0;
   size_t decA = 0;
   size_t foundOne = 0; /*while looking for slabs to free, did I find any instances still in use*/
   BERT_Slab* release_slab;/*current slab which can be released (while looking)*/
   BERT_Slab* release_slabs;/*list of slabs (pointer to last) which can be released*/
   ...
   if (vmark == maxVmark) {
      ...
      for (si = 0;si < berbmax;si++) {
         ...
         while (twrv_slab != NULL) {
            slabsVisited++;
            foundOne = 0;/*did I find any instances still in use in this slab*/
            slab = twrv_slab->slab;
            while (slab != NULL) {
               cursor = (void**) slab;
               /*this will set vsgen for never used instance positions, if cursor[berdef] == NULL you don't have to check
                or set the vsgen, it is a never before used position*/
               ...
               if ((*vs_gen & maxVmark) == vmark) {
                  ...
                  *vs_gen &= vmarkMask; /* *vs_gen = 0; */
                  if (asi == 1) {
                      /*need to free first slot in case the slab is freed*/
                      if (cursor[bercps] != NULL) {
                          BENoFree(cursor[bercps]);
                          cursor[bercps] = NULL;
                       }
                  }
            ...
            twrv_slab = twrv_slab->next;
            if (foundOne == 0 && twrv_slab != NULL && twrv_slab->prior->prior != NULL) {
               release_slab = twrv_slab->prior;
               twrv_slab->prior->prior->next = twrv_slab;
               twrv_slab->prior = twrv_slab->prior->prior;
               /*twrv_slab is incremented and release_slab is no longer meaningfully pointed to by anything else*/
               /*release_slab->next = NULL; unnecessary*/
               if (release_slabs == NULL) {
                   release_slab->prior = NULL;/*first one*/
               } else {
                   release_slab->prior = release_slabs;
               }
               release_slabs = release_slab;
            }
         }
      }
      }
...
x = NULL;/*the returned value from the named class's main*/
msg = NULL;/*only for exception during proc, used also for exit code determination*/
bevs = NULL;
berv_glob = BERV_proc_glob;
...
berv_sts->freeFirstAllocSlabs = (BERT_Slab**) BEMalloc(berbmax * sizeof(BERT_Slab*));
...
BERF_Init(berv_sts);
/*call the notNullInitializer here*/
berv_glob->mainNotNullInit(berv_sts);
berv_sts->passedClassDef = BEUV_6_16_SystemExceptionBuilder_clDef;
eb = BEKF_6_11_SystemInitializer_initializeIt_1(0, berv_sts, NULL, BERF_Create_Instance(berv_sts, berv_sts->passedClassDef, 0));
berv_sts->passBack = eb;
if (setjmp(twrv_stackex.env) == 0) {
twrv_sfc->except = &twrv_stackex;
berv_sts->passedClassDef = BEUV_4_3_MathInt_clDef;
ac = BEKF_4_3_MathInt_new_0(0, berv_sts, BERF_Create_Instance(berv_sts, berv_sts->passedClassDef, 0));
berv_sts->passBack = ac;
*((int*) ac + bercps) = argc;
berv_sts->passedClassDef = BEUV_6_5_SystemThing_clDef;
av = BEKF_6_5_SystemThing_new_0(0, berv_sts, BERF_Create_Instance(berv_sts, berv_sts->passedClassDef, 0));
berv_sts->passBack = av;
av[bercps] = (void*) argv;
berv_sts->passedClassDef = BEUV_6_7_SystemProcess_clDef;
ei = BEKF_6_11_SystemInitializer_initializeIt_1(0, berv_sts, NULL, BERF_Create_Instance(berv_sts, berv_sts->passedClassDef, 0));
...
twvp[0] = &(klass);
x = BEKF_6_7_SystemProcess_startByName_1(0, berv_sts, ei, klass);
} else {
...
#endif
if (msg != NULL) {
    return 1;/*we had an uncaught exception, exit non 0*/
}
/*if the returned value is not null and is an Int type, return it's value, else 0 for "normal completion"*/
if (x != NULL && ((BERT_ClassDef*) x[berdef]) == BEUV_4_3_MathInt_clDef) {
    return *((BEINT*) (x + bercps));
}
return 0;
}
...
int BERF_Start_Process_Free_Args(int argc, char **argv, char* klasschar, char* platchar) {
   ...
   result = BERF_Start_Process(argc, argv, klasschar, platchar);
   for (i = 0;i < argc;i++) {
      c_arg = argv[i];
      BENoFree(c_arg);
   }
